...
router.get("/post/:role/list", middleware._auth, async (req, res) => {
  ...
  try {
    if (role == 1 || role == 2) {
      posts = await _query(
        `SELECT p.*, u.gender, u.reputation FROM Post as p JOIN User as u ON p.student_id = u.student_id AND p.role=${role} AND is_matched = 0 
      ...
      posts = await _query(
        `SELECT p.*, u.gender, u.reputation FROM Post as p JOIN User as u ON p.student_id = u.student_id AND p.role=1 AND p.is_matched = 0
